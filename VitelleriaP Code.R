...
library(data.table) #data managment
library(ggplot2)    #plot and graph
library(ggpubr)# for ggpar,ggarrange
library(dismo)#for GBM dismo
library(neuralnet)#for aritficail neural network (ANN)
library(mapview) # for view data on open street and other mapping 
library(leaflet)
library(leafem)
...
#=========================================================================
setwd("C:/Users/gsalako/Documents/Vit_Model/RasterVit_Mosaic")
...
summary(ClVitdata)
#=========================================================================
##Spatial Thinning after duplictae removal
#=========================================================================

ThinVit1 <- thin(loc.data = ClVitdata, lat.col = "decimalLatitude", long.col = "decimalLongitude", spec.col = "Species", thin.par = 10, 
                 reps = 100, locs.thinned.list.return = TRUE, write.files = TRUE, max.files = 5, out.dir = "Vit_Thin_Full", 
                 out.base = "Vit_Thinned2", write.log.file = TRUE, log.file ="Vit_Thin_txt")
Vit_thin2 <- read.csv("Vit_Thinned2.csv") 
#convert saved thinned csv "Vit_thin2"to point data
proj4Str <- "+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs"
Vit_ThinPoints <- SpatialPointsDataFrame(coords = Vit_thin2[,c("decimalLongitude","decimalLatitude")], data = Vit_thin2, proj4string = CRS(proj4Str))
...
StackIndex=stack(NDVI,RVI,GNDVI,  EVI2, SAVI,  NDWI, MNDWI, MBwet,MBdry,MBrain)
#rename the raster
names(StackIndex) <-c("NDVI","GNDVI","SAVI", "NDWI", "MNDWI","EVI", "Wet", "Dry", "PPt","VitSDM")
